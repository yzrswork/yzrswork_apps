...
self.addEventListener('fetch', (event) => {
  ...
  // kitアプリの既知アセット以外は素通り（他ページに介入しない）
  if (!KIT_ASSET_URLS.has(url.href)) {
    return;
  }
...
//
// === キャッシュ範囲 ===
// このSWは /kit/ スコープで、ASSETS配列に列挙された既知アセットのみキャッシュする。
// 他ページ（memo.html, bench/, fixit/ 等）には介入しない。
