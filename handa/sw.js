...
const HANDA_ASSET_URLS = new Set(
...
self.addEventListener('fetch', (event) => {
  ...
  // handaアプリの既知アセット以外は素通り（他ページに介入しない）
  if (!HANDA_ASSET_URLS.has(url.href)) {
    return;
  }
...
//
// === キャッシュ範囲 ===
// このSWは /handa/ スコープで、ASSETS配列に列挙された既知アセットのみキャッシュする。
// 他ページには介入しない。
