...
const PINOUT_ASSET_URLS = new Set(
...
self.addEventListener('fetch', (event) => {
  ...
  // pinoutアプリの既知アセット以外は素通り（他ページに介入しない）
  if (!PINOUT_ASSET_URLS.has(url.href)) {
    return;
  }
...
//
// === キャッシュ範囲 ===
// このSWは /pinout/ スコープで、ASSETS配列に列挙された既知アセットのみキャッシュする。
// 他ページには介入しない。
